...
export class InstallService extends EventTarget {
  ...
  init() {
    ...
    console.info("[install]", this.diagnose());
  ...
  /**
   * Whether an offer that ignores a past refusal should be shown.
   *
   * "Не зараз" should quiet the cover, not remove the possibility.
   * Somewhere has to keep it, or the only way back is a console
   * command — which is the same as no way back for the people this is
   * built for.
   *
   * @returns {boolean}
   */
  ...
  reset() {
    ...
    this.#announce();
    console.info("[install] dismissal cleared", this.diagnose());
    return this;
  }

  /**
   * Everything that decides whether the offer appears.
   *
   * Install detection depends on browser behaviour that cannot be
   * observed from the outside — an event that may or may not have
   * fired, a display mode the browser reports on its own terms. When
   * the button is missing there is nothing to inspect, so this reports
   * each input separately rather than only the conclusion.
   *
   * @returns {object}
   */
  diagnose() {
    const dismissedUntil = this.#readDismissed();
    ...
    return {
      ...
      dismissedUntil: dismissedUntil ? new Date(dismissedUntil).toLocaleString("uk") : null,
      hasPrompt: Boolean(captured),
      platform: this.isIOS ? "ios" : this.isAndroid ? "android" : "desktop",
      engine: this.isChromium ? "chromium" : this.isSafari ? "safari" : "other",
