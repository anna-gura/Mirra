...
export class InstallPrompt {
  ...
  init() {
    ...
    if (this.#triggers.length === 0) {
      console.error(
        "InstallPrompt: no [data-install] buttons found — " +
        "are index.html and views/hub.tpl up to date?"
      );
    }
  ...
  /**
   * Shows or hides every trigger.
   *
   * Triggers marked data-install-keep survive a refusal. The one on the
   * cover is an interruption and goes quiet when told to; the one on
   * the hub is a menu item, and a menu that removes entries because
   * they were once declined is a menu people learn not to trust.
   */
  sync() {
    const available = this.#service.isAvailable;
    const possible = this.#service.isPossible;

    this.#triggers.forEach(trigger => {
      const keep = trigger.hasAttribute("data-install-keep");
      trigger.hidden = keep ? !possible : !available;
    });

    if (!possible) {
      console.info("[install] offer hidden —", this.#service.diagnose());
    }
